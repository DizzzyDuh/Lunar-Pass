...
export const createNewOrder= asyncHandler(async(req , res) => {
    ...
    const {user , passengers , shuttle ,  billingAddress , paymentMethod , taxPrice , totalPassengers , totalPrice } = req.body;
...
export const fetchOrderById = asyncHandler( async (req , res) => {
    ...
    const order = await Order.findById(id).populate('shuttle');
